...
mode = st.radio("حدد نطاق التحليل المطلوب:", ["🔎 معالجة كشوفات سنة منفردة", "⚖️ المقارنة الهيكلية وتحليل التباين بين سنتين"])
...
if mode == "🔎 معالجة كشوفات سنة منفردة":
    ...
    if file:
        ...
        if st.button("توليد الرؤية والتحليل الاستراتيجي الفوري 🤖"):
            if api_key:
                # الرابط الرسمي والمستقر المعتمد لعام 2026 لتفادي خطأ 404 تماماً
                ...
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        st.error(f"جوجل ترفض الطلب. الرمز: {res.status_code}. يرجى التحقق من صياغة المفتاح داخل Secrets.")
                except:
            ...
            if api_key:
                ...
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        st.error(f"جوجل ترفض الطلب. الرمز: {res.status_code}. يرجى مراجعة إعدادات Secrets.")
                except:
                    st.error("فشل محرك الاستعلام المالي، يرجى مراجعة الاتصال.")
            else:
                st.error("⚠️ لم يتم العثور على مفتاح الـ API المشفر في إعدادات السيرفر السريّة.")
